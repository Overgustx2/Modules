...
if not isfolder(`Astralise`) then
	makefolder(`Astralise`)
end
...
function Library:save_flags()
	...
	writefile(`Astralise/{game.GameId}.lua`, flags)
end


function Library:load_flags()
	if not isfile(`Astralise/{game.GameId}.lua`) then
		Library.save_flags()

		return
	end

	local flags = readfile(`Astralise/{game.GameId}.lua`)
...
function Library:new()
	local container = game:GetObjects('rbxassetid://17864117698')[1]
	...
	local mobile_button = game:GetObjects('rbxassetid://17868018725')[1]
